...
export class memberDecorator implements ICheck, IValidate {
    ...
    public type: 'property' | 'event' | 'method' | 'readonly';
    public isPrototype: boolean = true;
    befores: ITrigger[] = [];
    afters: ITrigger[] = []; //PushArray<ITrigger> = new PushArray<ITrigger>();// 
    checks: ICheck[] = [];
    onGets: ITrigger[] = [];
    validates: IValidate[] = [];
    ...
    public overwrite(d: memberDecorator) {
        ...
        if (this.getter != undefined) d.getter = this.getter;
        if (this.observableType) d.observableType = this.observableType;
        if (this.defaultPropertyValue !== undefined) d.defaultPropertyValue = this.defaultPropertyValue;
        d.type = this.type;
    ...
    public instantiate = () => {
        switch (this.type) {
            ...
            case 'method':
                ...
                break;
            case 'readonly':
                this.readonly(this.target, this.propertyKey);
                break;
        }
        this.setValuePaths(this.RealValue);
    ...
    public readonly = (target: Object, propertyKey: string, descriptor?: PropertyDescriptor) => {
        ...
        obs.setDecorator(target, propertyKey, this).initializeReadonly(target, propertyKey, descriptor);
    }
    public initializeReadonly = (target: Object, propertyKey: string, descriptor?: PropertyDescriptor) => {
        let that = this;
        this.initialize();
        this.type = 'readonly';
        if (descriptor) {
    ...
    private initializeProperty = (target: Object, propertyKey: string, descriptor?: PropertyDescriptor) => {
        ...
        if (this.defaultPropertyValue !== undefined) this.RealValue = this.defaultPropertyValue;
        if (descriptor) {
            descriptor.configurable = true;
    ...
    public initializeEvent = (target: Object, propertyKey: string, descriptor?: PropertyDescriptor) => {
        ...
        this.invoker = (...args: any[]): any => {
            ...
            let invokingArgs = [undefined, undefined, []];
    ...
    public allocate = (newDecorator: memberDecorator) => {
        ...
        if (newDecorator.defaultPropertyValue !== undefined) this.defaultPropertyValue = newDecorator.defaultPropertyValue;
    }
    public applyDecorator(d: memberDecorator) {
        ...
        if (d.defaultPropertyValue !== undefined) this.defaultPropertyValue = d.defaultPropertyValue;
...
export class objectDecorator {
    ...
    public in = (modulePath: string) => {
        this.modulePath = modulePath;
        return this;
    }
    private isReferenceType: boolean;
    ...
    public observable = (target: any) => {
        ...
        let arrayChangeHandler: ProxyHandler<ObservableArray<any>> = {
            deleteProperty: function (obsArray: ObservableArray<any>, property: string | number | symbol): boolean {
                ...
                if (obsArray.inOverwrittenOperation) return true; //do not raise event in case of internal operation;
 ...
 */
export class obs {
    static enableBindable(instance: any, classPrototype: Object) {
     ...
     * @param value
     */
    static stringifyReplacer(key: string, value: any) {
            ...
            for (let key in value) {
                switch (key) {
                    ...
                    case '@ObjectService.ModulePath':
                        replaced['@Serializable.ModuleName'] = value[key];
                        break;
                    case '@ObjectService.TypeName':
                        replaced['@Serializable.TypeName'] = value[key];
                        break;
                    default:
                        ...
                        if (d) {
                            ...
                            replaced[key] = value[key];
                        }
                        break;
                }
            }
            let decorators = value['@ObjectService.Decorators'];
            for (let key in decorators) {
                let d = decorators[key];
                if (d.type == 'property' && replaced[key] === undefined) {
                    replaced[key] = d.RealValue;
                }
            }
            return replaced;
        }
    ...
    static parseReviver(key: any, value: any){
        ...
        if (value['@Serializable.ModuleName'] && value['@Serializable.TypeName']) {
            ...
            System.import(value['@Serializable.ModuleName']).then((_mod) => {
                let _constructor = _mod[value['@Serializable.TypeName']];
                let replaced = new (Function.prototype.bind.apply(_constructor, []));
                for (let key in value) {
                    switch (key) {
    ...
    static appendMethodBefore(target: Object, member: (() => any) | string, host: Object, method: Function): ITrigger {
        let memberName = obp.analyzeMember(member);
        let decorator = obs.getDecorator(target, memberName, true);
        let trigger = { target: host, method: (source: Object, key: string, decorators: memberDecorator[], ...args: any[]) => method.apply(host, args) };
        if (decorator) decorator.befores.push(trigger);
    ...
    static in(modulePath: string): objectDecorator {
        return (new objectDecorator).in(modulePath);
    }
    static get ref(): objectDecorator {
    ...
    static makeObservable<T>(obj: ObservableArray<T>): ObservableArray<T> {
        let arrayChangeHandler: ProxyHandler<ObservableArray<any>> = {
            deleteProperty: function (obsArray: ObservableArray<any>, property: string | number | symbol): boolean {
                delete obsArray[property];
                if (obsArray.inOverwrittenOperation) return true;
                if (/^\d+$/ig.test(property.toString()) && obsArray.onchange) obsArray.onchange({ array: obsArray, operation: ArrayOperationType.delete, startIndex: Number(property) });
                return true;
            },
            get: function (obsArray: ObservableArray<any>, property: string) {
    ...
    static get readonly(): (target: Object, propertyKey: string, descriptor?: PropertyDescriptor) => void {
    ...
    static setDecorator(value: Object, propertyKey: string, decorator: memberDecorator): memberDecorator {
        ...
        if (decorators[propertyKey]) {
            ...
            if (decorators[propertyKey] != decorator) decorators[propertyKey].allocate(decorator);
        }
        else {
            decorators[propertyKey] = decorator;
        }
     ...
     * @param jsonObject
     */
    static async deserialize(jsonObject: any) {
        ...
        if (jsonObject['@ObjectService.ObservableArray']) {
            let arr = new ObservableArray<any>();
            let length = (<any[]>jsonObject).length;
            for (let i = 0; i < length; i++) {
                await arr.push(await obs.deserialize(jsonObject[i]));
            }
            return arr;
        }
        if (jsonObject['@ObjectService.GlobalType']) {
            ...
            switch (jsonObject['@ObjectService.GlobalType']) {
                case 'RegExp':
                    return new RegExp(jsonObject['source'], jsonObject['flags']);
                case 'Date':
                    return new Date(jsonObject['value']);
            }
        }
        if (jsonObject['@ObjectService.ModulePath'] && jsonObject['@ObjectService.TypeName']) {
            let moduleName: string = jsonObject['@ObjectService.ModulePath'];
            let typeName: string = jsonObject['@ObjectService.TypeName'];
            let module = await System.import(moduleName);
            let type: ObjectConstructor = module[typeName];
            let instance = new type();
            for (let property in jsonObject) {
                if (!/^@ObjectService\./.test(property)) {
                    //console.log('deserialize property: ', property, instance[property], jsonObject[property]);
                    if (instance[property] && instance[property]['@ObjectService.ObservableArray'] && Array.isArray(jsonObject[property])) {
                        let arr: any[] = (<any[]>(jsonObject[property]));
                        let obsArr: ObservableArray<any> = instance[property];
                        for (let i = 0; i < arr.length; i++) {
                            obsArr.push(await obs.deserialize(arr[i]));
                        }
                    }
                    else {
                        ...
                        try {
                            ...
                            let descriptor = Object.getOwnPropertyDescriptor(instance, property);
                            if (descriptor) {
                            ...
                            console.error('error at property: ', property, ex);
                        }
                    }
                }
                if (Array.isArray(jsonObject)) {
                    for (let i = 0; i < (<any[]>jsonObject).length; i++) {
                        instance[i] = await obs.deserialize(jsonObject[i]);
                    }
                }
            }
            return instance;
        ...
        }
        {
            ...
            for (let key in jsonObject) {
                instance[key] = await obs.deserialize(jsonObject[key]);
            }
            return instance;
        }
    ...
    private static serializationReplacer(key: string, value: any) {
        ...
        if (value['@ObjectService.ObservableArray']) {
            let arr = (<any[]>value).map(item => item);
            ...
            for (let key in value) {
                switch (key) {
                    ...
                    case '@ObjectService.Decorators':
                        break;
                    case '@ObjectService.ModulePath':
                        replaced['@ObjectService.ModulePath'] = value[key];
                        break;
                    case '@ObjectService.TypeName':
                        replaced['@ObjectService.TypeName'] = value[key];
                        break;
                    default:
                        let d = obs.getDecorator(value, key, true);
                        if (d) {
     ...
     * @param data
     */
    static pack(data: any) {
        let dict: any[] = [];

        function referenceReplacer(value: any) {
            if (!value) return value;
            if (typeof value != 'object') return value;
            if (value['@ObjectService.Reference']) {
                for (let index: number = 0; index < dict.length; index++) {
                    if (dict[index] == value) return { '@ObjectService.Pointer': index };
                }
                dict.push(value);
                ...
                for (let field in value) {
                    switch (field) {
                        ...
                        case '@ObjectService.TypeName':
                            ...
                            break;
                        default:
                            let d = obs.getDecorator(value, field, true);
                            if (d) {
    ...
    static async unpack(value: string) {
 ...
 */
class Unpacker {
    constructor(public dict: string[]) {
    }
    public async unpack() {
    ...
    public async getInstance(key: number) {
        ...
        let obj = JSON.parse(this.dict[key]);
        return await this.deserialize(obj, key);
    }
    public async deserialize(jsonObject: any, key?: number) {
        //console.log('deserialize: ', jsonObject);
        if (typeof jsonObject != 'object') return jsonObject;
        if (jsonObject['@ObjectService.ObservableArray']) {
            let instance = new ObservableArray<any>();
            if (typeof key == 'number') this.instances[key] = instance; //assign instance for async process;
            let length = (<any[]>jsonObject).length;
            for (let i = 0; i < length; i++) {
                await instance.push(await this.deserialize(jsonObject[i]));
            }
            return instance;
        }
        if (jsonObject['@ObjectService.GlobalType']) {
        ...
        if (typeof jsonObject['@ObjectService.Pointer'] == 'number') {
            return await this.getInstance(jsonObject['@ObjectService.Pointer']);
        }
        if (jsonObject['@ObjectService.ModulePath'] && jsonObject['@ObjectService.TypeName']) {
            let moduleName: string = jsonObject['@ObjectService.ModulePath'];
            let typeName: string = jsonObject['@ObjectService.TypeName'];
            ...
            for (let property in jsonObject) {
                if (!/^@ObjectService\./.test(property)) {
                    ...
                    if (instance[property] && instance[property]['@ObjectService.ObservableArray'] && Array.isArray(jsonObject[property])) {
                        ...
                        try {
                            ...
                            instance[property] = await this.deserialize(jsonObject[property]);
                        }
                        catch (ex) {
                            let descriptor = Object.getOwnPropertyDescriptor(instance, property);
                            if (descriptor) {
                ...
                if (Array.isArray(jsonObject)) {
                    for (let i = 0; i < (<any[]>jsonObject).length; i++) {
                        instance[i] = await this.deserialize(jsonObject[i]);
                    }
                }
            }
            return instance;
        ...
        }
        {
            ...
            if (typeof key == 'number') this.instances[key] = instance; //assign instance for async process;
            for (let key in jsonObject) {
                instance[key] = await this.deserialize(jsonObject[key]);
            }
            return instance;
        }
